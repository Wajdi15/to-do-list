...
mongoose.connect('mongodb+srv://admin-wajdi:123tEST@cluster0.941cx.mongodb.net/todolistDB',{useNewUrlParser:true,useUnifiedTopology:true, useFindAndModify: false })
const itemSchema = {
  name : String
}
const listSchema = {
...
Item.find({},(err,items)=>{
  if(err){
    console.log(err);
  }else if (items.length === 0){// for insert it one time
...
if(list === "Today"){
  ...
  List.findOneAndUpdate({name : list},{ $pull: { items :{ _id : deletedItem }}},(err,result)=>{
      res.redirect("/"+ list)
  })
}
});
...
//add new item
  ...
  if(listName === "Today"){
    ...
    List.findOne({name : listName},(err,list)=>{// find the list name and add item to the items table
